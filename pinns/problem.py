"""
ProblemStrong — Physics-Informed Neural Network problem definition.
...
from .strategies import StrategyUnique, StrategyFB, StrategyX, StrategyStep
...
class ProblemStrong:
    ...
    * Returns a scalar residual per point, or an array ``(n_points, n_eqs)``.
    ...
    def __init__(
        ...
        # ── stepper (StrategyStep acts as the stepper) ─────────────────────
        # For backward compat, a bare StrategyStep instance passed as
        # ``stepper=`` is also accepted and silently moved to ``strategy``.
        if stepper is not None:
            if not isinstance(stepper, StrategyStep):
                raise TypeError(
                    f"stepper must be a StrategyStep instance, got {type(stepper).__name__!r}."
                )
            if strategy is None:
                strategy = stepper
        ...
        self._stepper: Optional[StrategyStep] = (
            self._strategy if isinstance(self._strategy, StrategyStep) else None
        )
    ...
    def set_model(self, model: 'Network') -> 'ProblemStrong':
        """Attach a :class:`~pinns.network.Network` to this problem.
        ...
        :meth:`~pinns.network.Network.add_network_loss` and are independent
        of the problem.  The problem only holds physics residuals.

        Args:
            model: A :class:`~pinns.network.Network` instance.
        ...
        if not isinstance(model, Network):
            raise TypeError(
                f"model must be a Network instance, got {type(model).__name__!r}."
            )
        self._model = model
        model._setup(self.domain)
        return self
    ...
    @property
    def strategy(self):
        """Effective strategy for the Trainer.

        When a :class:`~pinns.network.Network` is attached via
        :meth:`set_model`, returns the network's temporal strategy
        (if set) or spatial strategy.  Falls back to the legacy
        ``strategy=`` argument passed to the constructor.
        """
        if self._model is not None:
            if self._model.temporal is not None:
                return self._model.temporal
            return self._model.spatial
    ...
    def add_dirichlet(
        ...
        :class:`~pinns.layers.Lifting` layer on the network instead.
    ...
    def add_dependency(
        ...
        * ``order``     — tuple of input-dimension indices specifying the
        ...
        Returns:
            ``self`` for method chaining.
        """
        if not isinstance(self._strategy, StrategyStep):
            raise TypeError(
                "add_dependency requires a model with a StrategyStep temporal strategy. "
                "Use a ModelStep/ModelXStep/ModelFBStep, or Model(net, temporal=StrategyStep())."
            )
